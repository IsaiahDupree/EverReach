...
async function main() {
  ...
  try {
    ...
    tests.push({
      ...
      status: res.status,
      ms,
      count: json?.interactions?.length || 0,
      note: json?.interactions?.length === 0 ? 'No interactions found (expected if no test data)' : null,
    });
    if (!pass) exitCode = 1;
  } catch (e) {
  ...
  if (testInteractionId) {
    try {
      const payload = { summary: 'Updated test interaction', sentiment: 'positive' };
      ...
      const pass = res.status === 200 && json?.interaction?.summary?.includes('Updated');
      tests.push({
        name: 'PATCH /v1/interactions/:id (update)',
        pass,
        status: res.status,
        ms,
        updated_summary: json?.interaction?.summary,
      });
      if (!pass) exitCode = 1;
    } catch (e) {
  ...
  try {
    ...
    tests.push({
      ...
      ms,
      count: json?.interactions?.length || 0,
      note: json?.interactions?.length === 0 ? 'No email interactions found (expected if no test data)' : null,
    });
    if (!pass) exitCode = 1;
  } catch (e) {
