...
async function main() {
  ...
  // Fix: Ensure base URL includes /api path for ensureContact
  const apiBase = BACKEND_BASE.includes('/api') ? BACKEND_BASE : `${BACKEND_BASE}/api`;
  const contact = await ensureContact({ base: apiBase, token, origin: TEST_ORIGIN, name: `Agent Compose ${id.slice(0,8)}` });
