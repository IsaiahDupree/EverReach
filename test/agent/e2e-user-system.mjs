...
async function main() {
  ...
  try {
    ...
    const pass = (res.status === 200 || res.status === 201) && json?.note?.id;
    if (pass) testPersonaNoteId = json.note.id;
    tests.push({
  ...
  if (testPersonaNoteId) {
    try {
      ...
      const pass = res.status === 200 && json?.note?.id === testPersonaNoteId;
      tests.push({
  ...
  if (testPersonaNoteId) {
    try {
      const payload = { content: 'Updated test persona note' };
