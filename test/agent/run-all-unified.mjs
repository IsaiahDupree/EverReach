/**
 * Unified Agent Test Runner
 * 
 * Runs all agent tests and generates a single comprehensive report with:
...
const UNIFIED_REPORT_PREFIX = 'unified_test_report';
...
function extractTestName(filePath) {
  return filePath.replace('test/agent/', '').replace('.mjs', '');
}

async function readTestReport(testName) {
  // Normalize: our per-test report prefixes use underscores, not hyphens
  const prefix = testName.split('/').pop(); // e.g., e2e-contacts-crud
  const normalized = prefix.replace(/-/g, '_'); // e2e_contacts_crud
  const runTag = process.env.TEST_RUN_ID ? `_run-${process.env.TEST_RUN_ID}` : '';
  try {
    const files = await readdir(REPORTS_DIR);
    // Prefer file for this run, otherwise latest matching prefix
    const matching = files
      .filter(f => f.startsWith(`${normalized}${runTag}_`) || (!runTag && f.startsWith(`${normalized}_`)))
      .sort() // timestamp in name ensures lexical sort works
      .reverse();
...
async function generateUnifiedReport(results) {
  ...
  lines.push('');
  lines.push('> **Note**: This section combines all individual test reports for comprehensive review.');
  lines.push('');

  // Include detailed reports from each test
  for (const result of sortedResults) {
    const testName = extractTestName(result.file);
    ...
    if (report) {
      ...
      const titleIndex = reportLines.findIndex(line => line.startsWith('# E2E Test:') || line.startsWith('# '));
      
      lines.push(`### ${result.code === 0 ? '✅' : '❌'} ${testName}`);
      lines.push('');
      
      if (titleIndex >= 0 && reportLines.length > titleIndex + 1) {
        // Skip the title, include everything else
      ...
      // No detailed report, show basic info
      lines.push(`### ${result.code === 0 ? '✅' : '❌'} ${testName}`);
      lines.push('');
  ...
  lines.push('');
  lines.push(`- **Backend**: ${process.env.NEXT_PUBLIC_API_URL || process.env.TEST_BASE_URL || 'N/A'}`);
  lines.push(`- **Origin**: ${process.env.TEST_ORIGIN || 'N/A'}`);
  lines.push(`- **Supabase**: ${process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || 'N/A'}`);
  ...
  lines.push('- ✅ Agent chat and conversation');
  lines.push('- ✅ Contact analysis and insights');
  lines.push('- ✅ Message composition and goals');
  lines.push('- ✅ Persona notes management');
  lines.push('- ✅ Screenshot analysis');
  lines.push('- ✅ Action suggestions');
  lines.push('- ✅ Tag updates');
  lines.push('- ✅ Interaction summaries');
  lines.push('- ✅ Cross-platform entitlements');
  lines.push('- ✅ AI context and actions');
  lines.push('');
  ...
  if (failed > 0) {
    ...
    lines.push('- Authentication failures');
...
async function main() {
  // Ensure a consistent RUN ID is used across all child processes so
  // per-test reports and the unified report can be correlated.
  if (!process.env.TEST_RUN_ID) {
    process.env.TEST_RUN_ID = Math.random().toString(36).slice(2, 10);
  }
  const dir = 'test/agent';
  const entries = await readdir(dir, { withFileTypes: true });
  const testFiles = entries
    .filter(e => e.isFile())
    .map(e => e.name)
    .filter(n => n.endsWith('.mjs') && n !== 'run-all.mjs' && n !== 'run-all-unified.mjs' && !n.startsWith('_'));

  if (testFiles.length === 0) {
    console.log('[agent-tests] No tests found in', dir);
    return;
  }

  console.log(`\n🧪 Running ${testFiles.length} agent tests...\n`);
...
main().catch((err) => {
  console.error('[agent-tests] Runner error:', err?.message || err);
  process.exit(1);
});
