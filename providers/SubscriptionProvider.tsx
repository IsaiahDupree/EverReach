...
import { addCustomerInfoUpdateListener, hasActiveEntitlement } from '@/lib/revenuecat';
...
export const [SubscriptionProvider, useSubscription] = createContextHook<SubscriptionState>(() => {
  ...
  const loadSubscriptionState = useCallback(async () => {
    try {
      ...
      if (isPaidSubscription) {
        ...
        // CRITICAL FIX: Set subscription start date - check in order:
        ...
        } else if (storedSubscriptionStart !== subscriptionStart) {
          // Backend has different value - update local storage
          await storage.setItem(STORAGE_KEYS.SUBSCRIPTION_START_DATE, subscriptionStart);
          console.log('[SubscriptionProvider] 🔄 Updated subscription start date from backend:', subscriptionStart);
        } else {
  ...
  // CRITICAL FIX: Auto-refresh entitlements when app comes back to foreground
  // This handles the case where user completes purchase in App Store and returns
  ...
  useEffect(() => {
    ...
    const handleAppStateChange = async (nextAppState: AppStateStatus) => {
      // Only refresh when app comes to foreground from background
      if (appState.match(/inactive|background/) && nextAppState === 'active') {
        const now = Date.now();
        const timeSinceLastRefresh = now - lastRefreshRef.current;
        
        // Throttle refreshes: minimum 5 seconds between auto-refreshes
        // This prevents excessive API calls while still being responsive
        if (timeSinceLastRefresh > 5000 && !isRefreshingRef.current) {
          console.log('[SubscriptionProvider] 🔄 App returned to foreground - auto-refreshing entitlements');
          ...
          try {
            ...
            console.warn('[SubscriptionProvider] ⚠️ Auto-refresh failed:', error);
          } finally {
  ...
  // CRITICAL FIX: Listen for RevenueCat CustomerInfo changes (real-time subscription updates)
  // This handles immediate purchase confirmation from RevenueCat
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      try {
        unsubscribe = await addCustomerInfoUpdateListener(async (customerInfo) => {
          console.log('[SubscriptionProvider] 🔔 RevenueCat CustomerInfo changed - checking entitlements');
          
          // Check if user now has active entitlements
          const hasEntitlement = await hasActiveEntitlement();
          
          if (hasEntitlement && !isPaid) {
            console.log('[SubscriptionProvider] 🎉 New entitlement detected! Refreshing state...');
            lastRefreshRef.current = Date.now();
            await loadSubscriptionState();
          } else if (!hasEntitlement && isPaid) {
            console.log('[SubscriptionProvider] ⚠️ Entitlement lost - refreshing state...');
            lastRefreshRef.current = Date.now();
            await loadSubscriptionState();
          }
        });
        console.log('[SubscriptionProvider] ✅ RevenueCat listener registered');
      } catch (error) {
        console.warn('[SubscriptionProvider] Failed to setup RevenueCat listener:', error);
      }
    };
