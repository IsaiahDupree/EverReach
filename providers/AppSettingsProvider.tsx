...
export const [AppSettingsProvider, useAppSettings] = createContextHook(() => {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        const [syncEnabledStr, trialStartStr, isPaidStr, themeModeStr, cloudModeStr] = await Promise.all([
        ...
        if (cloudModeStr !== null) {
          try {
            ...
            AsyncStorage.multiSet([
              [CLOUD_MODE_KEY, JSON.stringify(initialCloudMode)]
            ]).catch(() => {});
          }
        } else {
          if (isMounted) setCloudModeEnabled(initialCloudMode);
          AsyncStorage.setItem(CLOUD_MODE_KEY, JSON.stringify(initialCloudMode)).catch(() => {});
        }
  ...
  return useMemo(() => ({
    ...
    enableCloudMode,
    disableCloudMode
  }), [syncEnabled, enableSync, disableSync, trialDaysLeft, isPaid, themeMode, setTheme, currentTheme, cloudModeEnabled, enableCloudMode, disableCloudMode]);
});