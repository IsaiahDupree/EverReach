...
async function evaluateCampaign(campaign: any): Promise<number> {
  console.log(`[run-campaigns] Evaluating campaign: ${campaign.name}`);
  
  try {
    // Execute the entry SQL (returns user_id, variant_key, reason, context_json)
...
serve(async (req) => {
  try {
    ...
    if (!campaigns || campaigns.length === 0) {
      ...
      return new Response(
        JSON.stringify({ ok: true, message: "No campaigns enabled" }),
    ...
    for (const campaign of campaigns) {
      const count = await evaluateCampaign(campaign);
...
// Helper function to create (must exist in database)
// CREATE OR REPLACE FUNCTION exec_campaign_sql(sql text)
// RETURNS jsonb AS $$
// BEGIN
//   RETURN (SELECT jsonb_agg(row_to_json(t)) FROM (EXECUTE sql) t);
// END;
