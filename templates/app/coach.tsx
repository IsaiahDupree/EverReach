/**
 * SunTrace - AI Coach Screen
 *
 * Features:
 * - Chat UI with user/assistant message bubbles
 * - Medical disclaimer banner
 * - Pro gated (first tap shows paywall for free users)
 * - Calls Supabase Edge Function /functions/v1/coach-chat
...
import { Sun, Send, Lock, AlertTriangle, ChevronRight } from 'lucide-react-native';
import { getCoachMessages, sendCoachMessage, getTodayCoachMessageCount } from '@/services/api';
    ...
    <View style={gateStyles.container}>
      <View style={gateStyles.card}>
        ...
        <Text style={gateStyles.subtitle}>
          Get personalized Vitamin D advice, safe sun exposure recommendations, and answers to all your sunlight questions.
        </Text>
...
function LimitBanner({
  ...
  return (
    <TouchableOpacity style={limitStyles.banner} onPress={onUpgrade} activeOpacity={0.85}>
      ...
      {remaining === 0 && (
        <Text style={limitStyles.link}>Upgrade</Text>
      )}
    </TouchableOpacity>
  );
}
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.countText}>{todayCount}/{limit}</Text>
      </View>
