...
import PipelineHistoryTimeline from "@/features/contacts/components/PipelineHistoryTimeline";
...
export default function ContactDetailScreen() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorHint}>Contact ID: {id}</Text>
          <TouchableOpacity 
  ...
  // Bottom navigation is unified via DashboardBottomNav utility
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: themeColors.primary }]}>
        <TouchableOpacity style={styles.backNav} onPress={() => safeGoBack(router)}>
      ...
      <ContactChannels
        ...
        socialChannels={contact.social_channels || []}
        preferredChannels={(contact as any).comms?.channelsPreferred || []}
        onUpdateSocialChannels={async (channels: SocialMediaChannel[]) => {
          try {
            ...
            // Always include social_channels if we have valid data
            if (validChannels.length > 0) {
              updatePayload.social_channels = validChannels;
            } else {
              // If no valid channels, send empty array explicitly
              updatePayload.social_channels = [];
            }
        ...
        onSetPreferredChannel={async (platform: string) => {
          try {
            const current: string[] = ((contact as any).comms?.channelsPreferred || []) as string[];
            ...
            const response = await apiFetch(`/api/v1/contacts/${contact.id}`, {
              method: 'PATCH',
              requireAuth: true,
              body: JSON.stringify({ comms: { channelsPreferred: next } }),
              headers: { 'Content-Type': 'application/json' },
            });
            if (!response.ok) {
              const msg = await response.text();
              throw new Error(`Failed to set preferred channel: ${response.status} ${msg}`);
            }
            await refetchAll();
          } catch (e) {
            console.error('[ContactDetail] Failed to set preferred social channel:', e);
            throw e;
          }
        }}
        editable={true}
      ...
      <View style={styles.quickActionsSection}>
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            go.voiceNote(contact.id, contact.display_name);
          }}
        >
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            go.screenshotAnalysis(contact.id);
          }}
        >
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            go.chatWithPrompt(`Tell me more about ${contact.display_name}`);
          }}
        >
      ...
      <View style={styles.craftMessageSection}>
        <TouchableOpacity 
          ...
          testID="craftMessageButton"
          onPress={() => go.goalPicker(contact.id, 'sms')}
        >
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => go.context(contact.id)}
          testID="contextSummaryTile"
        >
          ...
          {contextSummary?.interests && contextSummary.interests.length > 0 && (
            <View style={styles.contextSection}>
              <Text style={styles.contextSubtitle}>Interests & Tags:</Text>
              <View style={styles.contextChips}>
                {contextSummary.interests.map((interest) => (
      ...
      {pipelineHistory.length > 0 && (
        <PipelineHistoryTimeline
          history={pipelineHistory}
          currentStage={pipelineState?.stage_key || undefined}
        />
      )}
      ...
      <View style={styles.bottomActions}>
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            go.to(`/add-contact?editId=${contact.id}`);
          }}
        >
      ...
      >
        <View style={styles.stickyRow}>
          <TouchableOpacity style={styles.stickyBack} onPress={() => safeGoBack(router)}>
          ...
          <TouchableOpacity
            ...
            testID="craftMessageButtonSticky"
            onPress={() => go.goalPicker(contact.id, 'sms')}
            activeOpacity={0.9}
          >
