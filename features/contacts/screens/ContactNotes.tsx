...
import { go, safeGoBack } from '@/lib/navigation';
...
export default function ContactNotesScreen() {
  ...
  // Analytics tracking
  const screenAnalytics = useAnalytics('ContactNotes');
  ...
  const [supabaseNotes, setSupabaseNotes] = useState<any[]>([]);
  const [isLoadingSupabaseNotes, setIsLoadingSupabaseNotes] = useState(false);

  const fetchSupabaseNotes = React.useCallback(async () => {
    if (!id || FLAGS.LOCAL_ONLY) return;
    
    setIsLoadingSupabaseNotes(true);
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Fetched Supabase notes:', notes.length);
      } else {
        console.error('Failed to fetch Supabase notes:', response.status);
      }
    } catch (error) {
      console.error('Error fetching Supabase notes:', error);
    } finally {
      setIsLoadingSupabaseNotes(false);
    }
  }, [id]);
  ...
  const supabaseInteractions: Interaction[] = supabaseNotes.map((note: any) => ({
    id: note.id,
    channel: 'note' as InteractionChannel,
    occurredAt: note.created_at,
    summary: note.content || '',
    sentiment: undefined
  }));

  const localInteractions = person?.interactions || [];

  const mergedInteractions = FLAGS.LOCAL_ONLY 
    ? localInteractions 
    : [...supabaseInteractions, ...localInteractions].reduce((acc, curr) => {
  ...
  const voiceInteractions: (Interaction & { type: 'voice' })[] = personVoiceNotes.map(vn => ({
    id: vn.id,
    channel: 'note' as const,
    occurredAt: new Date(vn.createdAt).toISOString(),
    summary: vn.transcription || 'Voice note (no transcript)',
    sentiment: undefined,
    type: 'voice' as const
  }));

  const allInteractions: (Interaction & { type: 'interaction' | 'voice' })[] = [
    ...mergedInteractions.map(i => ({ ...i, type: 'interaction' as const })),
    ...voiceInteractions
  ...
  const handleAddNote = async () => {
    if (noteInput.trim()) {
      ...
      // Track note added
      screenAnalytics.track('contact_note_added', {
        contactId: person.id,
      ...
      // Sync to backend if not in local-only mode
      if (!FLAGS.LOCAL_ONLY) {
        try {
          const response = await apiFetch(`/api/v1/contacts/${person.id}/notes`, {
            method: 'POST',
            requireAuth: true,
            body: JSON.stringify({
              content: noteInput.trim(),
              metadata: {
                source: 'contact-notes-page',
          ...
          if (response.ok) {
            ...
            console.error('Failed to save note to Supabase:', response.status);
          }
        } catch (error) {
          console.error('Error saving note to backend:', error);
          analytics.errors.occurred(error as Error, 'ContactNotes');
        }
      }
    }
  };
  ...
  const handleEditNote = async (noteId: string, newText: string) => {
    if (newText.trim()) {
      // Track note edited
      screenAnalytics.track('contact_note_edited', {
        contactId: person.id,
        noteId: noteId,
        noteLength: newText.trim().length,
      });
      ...
      // Note: Backend doesn't support editing notes via API yet
      // This only updates local state
      console.log('⚠️ Note edited locally only (backend edit not supported)');
    }
    setEditingNote(null);
    setEditText('');
  };
  ...
  const handleDeleteNote = async (noteId: string) => {
    try {
      // Track note deleted
      screenAnalytics.track('contact_note_deleted', {
        contactId: person.id,
        noteId: noteId,
      });
      ...
      // Note: Backend doesn't support deleting notes via API yet
      // This only updates local state
      console.log('✅ Note deleted locally');
    } catch (error) {
      console.error('[ContactNotes] Delete note error:', error);
      analytics.errors.occurred(error as Error, 'ContactNotes');
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
            ...
            <View style={styles.timelineSection}>
              ...
              {isLoadingSupabaseNotes && !FLAGS.LOCAL_ONLY ? (
