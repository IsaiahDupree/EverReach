...
import { parseVoiceNoteParams } from "@/lib/params";
...
export default function VoiceNoteScreen() {
  ...
  const { personId, personName } = parseVoiceNoteParams(rawParams as any);
  ...
  const [selectedPersonIds, setSelectedPersonIds] = useState<string[]>(() => (personId ? [personId as string] : []));
  ...
  // Toggle interest selection for tagging
  ...
  const startRecording = async () => {
    try {
      ...
      screenAnalytics.track('recording_started', {
        contactId: selectedPersonIds[0] || undefined,
        contactIds: selectedPersonIds,
        contextScope,
      });
      ...
      if (Platform.OS === 'web') {
        ...
        recorder.ondataavailable = (e) => { if (e.data && e.data.size > 0) webChunksRef.current.push(e.data); };
        recorder.start(100);
        setIsRecording(true);
        ...
        const updateMeter = () => {
          ...
          setAudioLevel(level); animateBars(level);
          webAnimationFrameRef.current = requestAnimationFrame(updateMeter);
        };
        updateMeter();
      } else {
        const permission = await Audio.requestPermissionsAsync();
        if (!permission.granted) { alert('Permission to access microphone is required!'); return; }
        ...
        await rec.prepareToRecordAsync({
          ...
          ios: { extension: '.wav', outputFormat: Audio.IOSOutputFormat.LINEARPCM, audioQuality: Audio.IOSAudioQuality.HIGH, sampleRate: 44100, numberOfChannels: 2, bitRate: 128000, linearPCMBitDepth: 16, linearPCMIsBigEndian: false, linearPCMIsFloat: false },
          web: { mimeType: 'audio/webm', bitsPerSecond: 128000 },
        });
        rec.setOnRecordingStatusUpdate((status) => {
          if (status.isRecording && (status as any).metering) {
            const normalized = Math.max(0, Math.min(1, (((status as any).metering) + 160) / 160));
            setAudioLevel(normalized); animateBars(normalized);
          }
        });
...
const stopRecording = async () => {
  ...
  try {
    if (Platform.OS === 'web') {
      ...
      if (webAnimationFrameRef.current) { cancelAnimationFrame(webAnimationFrameRef.current); webAnimationFrameRef.current = null; }
      ...
      webRecorderRef.current = null; webMediaStreamRef.current = null; webAudioCtxRef.current = null; webAnalyserRef.current = null; webChunksRef.current = [];
    } else {
      if (!recording) return;
      ...
      try { await FileSystem.moveAsync({ from: uri, to: newUri }); audioUri = newUri; } catch { audioUri = uri; }
      setRecording(null);
    }
    setRecordedAudioUri(audioUri); setRecordedAudioBlob(audioBlob);
    if (!audioUri && !audioBlob) return;
    setIsTranscribing(true);
    const form = new FormData();
    if (Platform.OS === 'web' && audioBlob) { form.append('audio', audioBlob, 'recording.webm'); }
    else if (audioUri) { const parts = audioUri.split('.'); const ext = parts[parts.length - 1]; const file: any = { uri: audioUri, name: `recording.${ext}`, type: `audio/${ext}` }; form.append('audio', file); }
    const baseUrl = process.env.EXPO_PUBLIC_API_URL || 'https://ever-reach-be.vercel.app';
    const resp = await fetch(`${baseUrl}/api/v1/transcribe`, { method: 'POST', body: form });
    const json = await resp.json(); if (!resp.ok) throw new Error(json.error || 'Transcription failed');
    const text: string = json.text || ''; setTranscript(text);
    const recordingDuration = recordingStartTime > 0 ? Date.now() - recordingStartTime : duration * 1000;
    screenAnalytics.track('recording_completed', { contactId: selectedPersonIds[0] || undefined, contactIds: selectedPersonIds, duration: Math.floor(recordingDuration / 1000), transcriptLength: text.length });
    const extracted = INTEREST_SUGGESTIONS.filter((i) => text.toLowerCase().includes(i.toLowerCase())).slice(0, 6);
    setSelectedInterests(extracted); setShowInterestTags(extracted.length > 0 || text.length > 50);
    screenAnalytics.track('transcription_viewed', { contactId: selectedPersonIds[0] || undefined, contactIds: selectedPersonIds, transcriptLength: text.length, duration: Math.floor(recordingDuration / 1000) });
    if (extracted.length > 0) { screenAnalytics.track('contacts_extracted', { contactId: selectedPersonIds[0] || undefined, contactIds: selectedPersonIds, extractedCount: extracted.length, interests: extracted }); }
  } catch (error) {
    console.error('Recording/transcription error:', error); analytics.errors.occurred(error as Error, 'VoiceNote');
    const recDur = recordingStartTime > 0 ? Date.now() - recordingStartTime : duration * 1000;
    screenAnalytics.track('recording_cancelled', { contactId: selectedPersonIds[0] || undefined, contactIds: selectedPersonIds, duration: Math.floor(recDur / 1000), reason: 'error' });
    alert('Failed to process recording. Please try again.');
  } finally {
    setIsTranscribing(false); setAudioLevel(0); barsAnim.forEach((bar) => bar.setValue(0.1));
  }
};
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (contextScope === 'about_person' && selectedPersonIds.length > 0) {
        // Save voice note to each selected contact
        for (const pid of selectedPersonIds) {
          const created = await VoiceNotesRepo.create({
            personId: pid,
            audioUri: recordedAudioUri || '',
            transcription: transcript,
            ...(recordedAudioBlob ? { audioBlob: recordedAudioBlob } : {}),
          });
          // If we attempted audio but created note has no hosted URL, notify fallback
          if ((recordedAudioUri || recordedAudioBlob) && !(created.audioUri || '').match(/^https?:\/\//i)) {
            alert('Saved as text-only note. (Upload unavailable)');
          }
    ...
    <AuthGate requireAuth>
      <SafeAreaView style={styles.container}>
          ...
          {contextScope === 'about_person' && (
            <View style={styles.section}>
              ...
              <ContactPicker
                ...
                onChangeSelected={setSelectedPersonIds}
        ...
        <View style={styles.actions}>
          ...
          <TouchableOpacity 
            style={[styles.saveButton, (saving || !transcript || (contextScope === 'about_person' && selectedPersonIds.length === 0)) && styles.saveButtonDisabled]}
            onPress={handleSave}
            disabled={saving || !transcript || (contextScope === 'about_person' && selectedPersonIds.length === 0)}
          >
