-- ============================================================================
-- Personal Profile API - Manual Migration Application (FIXED)
...
DROP POLICY IF EXISTS "Users can view own compose settings" ON compose_settings;
CREATE POLICY "Users can view own compose settings"
  ON compose_settings FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own compose settings" ON compose_settings;
CREATE POLICY "Users can insert own compose settings"
  ON compose_settings FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own compose settings" ON compose_settings;
...
-- Drop existing policies if they exist, then create new ones
DROP POLICY IF EXISTS "Users can view own persona notes" ON persona_notes;
CREATE POLICY "Users can view own persona notes"
  ON persona_notes FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own persona notes" ON persona_notes;
CREATE POLICY "Users can insert own persona notes"
  ON persona_notes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own persona notes" ON persona_notes;
CREATE POLICY "Users can update own persona notes"
  ON persona_notes FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own persona notes" ON persona_notes;
