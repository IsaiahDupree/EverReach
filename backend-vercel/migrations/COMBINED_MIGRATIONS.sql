/**
 * Combined Migrations - Apply All Pending Changes
 * 
 * Copy/paste this entire file into Supabase SQL Editor and execute
 * 
 * Includes:
 * 1. User Bio feature (for AI personalization)
 * 2. Contact Photo Download/Re-hosting system
 * 
 * Date: November 7, 2025
 */

-- ============================================================================
-- MIGRATION 1: USER BIO
-- ============================================================================
...
COMMENT ON COLUMN profiles.bio IS 'User bio/description - used in AI message generation and goal suggestions for personalization';

-- Full-text search index for bios (optional, for future search features)
CREATE INDEX IF NOT EXISTS idx_profiles_bio 
ON profiles USING gin(to_tsvector('english', bio)) 
WHERE bio IS NOT NULL;

-- ============================================================================
-- MIGRATION 2: CONTACT PHOTO DOWNLOAD JOBS
-- ============================================================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '✅ User bio column added to profiles table';
  ELSE
    RAISE WARNING '❌ User bio column NOT found in profiles table';
  END IF;

  -- Check contact_photo_jobs table exists
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_name = 'contact_photo_jobs'
  ) THEN
    RAISE NOTICE '✅ contact_photo_jobs table created';
  ELSE
    RAISE WARNING '❌ contact_photo_jobs table NOT found';
  END IF;

  -- Check queue function exists
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'queue_contact_photo_download'
  ) THEN
    RAISE NOTICE '✅ queue_contact_photo_download function created';
  ELSE
    RAISE WARNING '❌ queue_contact_photo_download function NOT found';
  END IF;
END $$;
...
COMMENT ON SCHEMA public IS 'Migrations applied: User Bio + Contact Photo Jobs (2025-11-07)';
