...
-- This script safely drops and recreates the templates table
...
CREATE TABLE templates (
  ...
  -- Constraints
  CONSTRAINT valid_channel CHECK (channel IN ('email', 'sms', 'dm', 'any')),
...
CREATE INDEX idx_templates_org_id ON templates(org_id) WHERE org_id IS NOT NULL;
...
CREATE INDEX idx_templates_is_favorite ON templates(user_id, is_favorite) WHERE is_favorite = TRUE;
CREATE INDEX idx_templates_created_at ON templates(created_at DESC);
CREATE INDEX idx_templates_usage ON templates(usage_count DESC, last_used_at DESC NULLS LAST);
...
CREATE MATERIALIZED VIEW mv_popular_templates AS
SELECT 
  t.id,
  t.name,
  t.category,
...
-- Sample templates (only insert if users exist)
DO $$
BEGIN
