...
CREATE INDEX idx_api_keys_active ON api_keys (organization_id, revoked) WHERE revoked = false;
...
CREATE INDEX idx_api_audit_logs_org ON api_audit_logs (organization_id, created_at DESC);
...
CREATE INDEX idx_automation_rules_type ON automation_rules (organization_id, type) WHERE enabled = true;
...
CREATE INDEX idx_outbox_org ON outbox (organization_id, status);
CREATE INDEX idx_outbox_contact ON outbox (contact_id);
CREATE INDEX idx_outbox_approval ON outbox (organization_id, status) WHERE status = 'awaiting_approval';
...
-- Verify API key (returns organization_id and scopes if valid)
