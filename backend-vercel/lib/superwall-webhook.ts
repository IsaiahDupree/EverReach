...
/**
 * Verify Superwall/Svix webhook signature
 * Superwall uses Svix for webhook delivery with these headers:
 * - svix-id: Unique message ID
 * - svix-timestamp: Unix timestamp
 * - svix-signature: HMAC signature(s)
 */
export function verifySuperwallSignature(
  payload: string,
  svixHeaders: { id: string | null; timestamp: string | null; signature: string | null },
  secret: string | undefined
): boolean {
  const { id: msgId, timestamp: msgTimestamp, signature: msgSignature } = svixHeaders;
  
  if (!msgId || !msgTimestamp || !msgSignature || !secret) {
    console.warn('[Superwall] Missing Svix headers or secret for verification');
    console.warn('[Superwall] Headers:', { msgId: !!msgId, msgTimestamp: !!msgTimestamp, msgSignature: !!msgSignature, secret: !!secret });
    return false;
  }
  ...
  try {
    ...
    if (Math.abs(now - timestamp) > 300) {
      console.warn('[Superwall] Webhook timestamp too old or in future:', { timestamp, now, diff: now - timestamp });
      return false;
    }

    // Create the signed content: msgId.msgTimestamp.payload
    ...
    // Extract all signatures from header (space-separated)
    ...
    console.warn('[Superwall] No matching signature found');
    console.warn('[Superwall] Expected:', expectedSignature);
    console.warn('[Superwall] Received:', passedSignatures);
    return false;
  } catch (error) {
...
/**
 * Legacy signature verification (kept for backwards compatibility)
 */
export function verifySuperwallSignatureLegacy(
  payload: string,
