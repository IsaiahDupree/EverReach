...
export const contactsListQuerySchema = z.object({
  ...
  sort: z.enum(['created_at.desc','created_at.asc','warmth.desc','warmth.asc','warmth_score_current.desc','warmth_score_current.asc']).default('created_at.desc').optional(),
...
export const personaNoteCreateSchema = z.object({
  ...
  tags: z.array(z.string().min(1).max(40)).max(50).optional(),
}).refine((o) => (o.type === 'text' && !!o.body_text) || (o.type === 'voice' && !!o.file_url), {
...
export const composeSettingsUpdateSchema = z.object({
  ...
  // Allow both enum values and custom text (like "Casual")
  tone: z.union([
    z.enum(['concise','warm','professional','playful']),
    z.string().max(500) // Custom tone text
  ]).optional(),
