...
export async function POST(req: Request, { params }: { params: { id: string } }){
  ...
  try {
    ...
    // Insert history (include org_id)
    const { error: hErr } = await supabase
      .from('contact_pipeline_history')
      .insert([{ contact_id: params.id, org_id: contact.org_id, pipeline_id: state.pipeline_id, from_stage_id: state.stage_id, to_stage_id: parsed.data.stage_id, changed_by_user_id: user.id, reason: parsed.data.reason ?? null }]);
