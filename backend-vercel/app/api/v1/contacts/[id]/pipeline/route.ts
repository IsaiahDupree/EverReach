...
export async function POST(req: Request, { params }: { params: { id: string } }){
  ...
  try {
    ...
    const { data: state, error: upErr } = await supabase
      .from('contact_pipeline_state')
      .upsert({ contact_id: params.id, pipeline_id: pipeline.id, stage_id: stageId }, { onConflict: 'contact_id' } as any)
    ...
    const { error: hErr } = await supabase
      ...
      .insert([{ contact_id: params.id, pipeline_id: pipeline.id, from_stage_id: null, to_stage_id: stageId, changed_by_user_id: user.id, reason: 'set' }]);
...
// PUT alias — mobile may send PUT for updates
