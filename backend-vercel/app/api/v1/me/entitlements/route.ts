...
export async function GET(req: Request) {
  ...
  let subscription_started_at: string | null = null;

  // 1) Check subscriptions table first (RevenueCat/App Store purchases)
  // Query all columns with * to see what exists
  let subscription: { status?: string | null; current_period_end?: string | null; product_id?: string | null } | null = null;
  try {
  ...
  if (isRevenueCatActive) {
    ...
    // Map revenuecat to specific store based on store field or product_id
    const storeField = (subscription as any)?.store;
    if (storeField === 'app_store' || storeField === 'play' || storeField === 'stripe') {
      source = storeField;
    } else if (productId.includes('ios') || productId.startsWith('com.')) {
      source = 'app_store';
    } else if (productId.includes('android')) {
      source = 'play';
    } else {
      // Default to app_store for RevenueCat subscriptions without clear indicator
      source = 'app_store';
    }
    ...
    // Get subscription start date from subscriptions table
    subscription_started_at = (subscription as any)?.started_at || null;
  } else {
    // Check Stripe
