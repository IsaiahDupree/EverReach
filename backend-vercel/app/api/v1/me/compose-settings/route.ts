...
export async function PATCH(req: Request){
  ...
  // Debug logging for tone
  if (parsed.data.tone !== undefined) {
    console.log('[ComposeSettings] Saving tone:', {
      userId: user.id,
      tone: parsed.data.tone,
      toneType: typeof parsed.data.tone,
      toneLength: parsed.data.tone?.length
    });
  }
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      console.error('[ComposeSettings] Error saving:', error);
      return serverError(error.message, req);
    }
    
    // Log what was actually saved
    if (data?.tone !== undefined) {
      console.log('[ComposeSettings] ✅ Tone saved successfully:', {
        userId: user.id,
        savedTone: data.tone,
        toneType: typeof data.tone
      });
    }
    
    return ok({ settings: data }, req);
  } catch (e: any) {
    console.error('[ComposeSettings] Exception:', e);
    return serverError(e?.message || 'Internal error', req);
  }
}
