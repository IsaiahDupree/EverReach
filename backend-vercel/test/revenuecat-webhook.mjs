...
async function test1_SignatureVerification() {
  ...
  const { response: invalidResp, data: invalidData } = await sendWebhookEvent(event, { invalidSignature: true });
  assert(invalidResp.status === 401, `Expected 401 for invalid signature, got ${invalidResp.status}`);
  assert(invalidData.ok === false, 'Should reject invalid signature');
  logOk('Invalid signature rejected');
