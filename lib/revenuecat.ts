...
// Customer info change listener for real-time subscription updates
type CustomerInfoListener = (info: any) => void;
let customerInfoListeners: CustomerInfoListener[] = [];
let isListenerSetup = false;

export async function addCustomerInfoUpdateListener(listener: CustomerInfoListener): Promise<() => void> {
  customerInfoListeners.push(listener);
  
  // Setup the native listener only once
  if (!isListenerSetup && Platform.OS !== 'web' && Constants.appOwnership !== 'expo') {
    try {
      const Purchases = await loadPurchases();
      if (Purchases && typeof Purchases.addCustomerInfoUpdateListener === 'function') {
        Purchases.addCustomerInfoUpdateListener((info: any) => {
          console.log('[RevenueCat] 🔔 CustomerInfo updated - notifying listeners');
          customerInfoListeners.forEach(l => {
            try {
              l(info);
            } catch (e) {
              console.warn('[RevenueCat] Listener error:', e);
            }
          });
        });
        isListenerSetup = true;
        console.log('[RevenueCat] ✅ CustomerInfo listener setup');
      }
    } catch (e) {
      console.warn('[RevenueCat] Failed to setup CustomerInfo listener:', e);
    }
  }
  
  // Return unsubscribe function
  return () => {
    customerInfoListeners = customerInfoListeners.filter(l => l !== listener);
  };
}

// Check if user has active entitlements from RevenueCat
export async function hasActiveEntitlement(): Promise<boolean> {
  try {
    ...
    return activeEntitlements && Object.keys(activeEntitlements).length > 0;
  } catch (e) {
    console.warn('[RevenueCat] hasActiveEntitlement error:', e);
    return false;
  }
}
