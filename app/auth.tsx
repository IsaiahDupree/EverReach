...
import { trackRegistrationEvent, trackTrialStartEvent } from '@/lib/metaServerEvents';
...
export default function Auth() {
  ...
  // Pre-fill email and isSignUp from URL params
  useEffect(() => {
    ...
    if (isSignUpParam && !userChangedSignUp.current) {
      setIsSignUp(true);
    }
  ...
  }, [params.email, params.isSignUp, params.plan]);
  ...
  const handlePasswordAuth = async (manualEmail?: string, manualPassword?: string) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Track registration with Meta (server-side)
        trackRegistrationEvent(targetEmail.trim(), undefined, 'email');
        trackTrialStartEvent(targetEmail.trim(), undefined, 7);
