...
export default function SettingsScreen() {
  ...
  // CRITICAL FIX: Auto-refresh entitlements when settings screen comes into focus
  // This ensures subscription status is always current when user views settings
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // Only refresh if more than 10 seconds since last refresh (avoid rapid re-fetches)
      if (timeSinceLastRefresh > 10000) {
        console.log('[Settings] 🔄 Screen focused - refreshing entitlements silently');
        lastFocusRefreshRef.current = now;
        
        // Silent refresh - don't show loading indicator
        refreshEntitlements().catch((error) => {
          console.warn('[Settings] Silent entitlement refresh failed:', error);
        });
      }
    }, [refreshEntitlements])
  );
  ...
  const handleSignOut = () => {
    ...
    const performSignOut = async () => {
      try {
        ...
        // WEB: Also clear any Supabase session keys from localStorage
        if (Platform.OS === 'web' && typeof window !== 'undefined' && window.localStorage) {
          ...
          keysToRemove.forEach(key => {
            ...
            console.log('[Settings] Cleared localStorage key:', key);
          });
        }
        ...
        // WEB: Redirect to landing page (not onboarding-v2)
        // NATIVE: Redirect to onboarding-v2 which will show S1 (Welcome) screen
        if (Platform.OS === 'web') {
          try { 
