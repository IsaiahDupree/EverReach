...
// Platform.OS can be 'web' at runtime even though TypeScript types don't include it
// MUST be defined before conditional imports
const IS_WEB = (Platform.OS as string) === 'web';

// These imports are mocked on web via metro.config.js
...
import { initializeMetaPixel, initializeFbcCapture } from "@/lib/metaPixel";
...
// Only run these initializations in browser, not during SSR/static rendering
if (IS_BROWSER) {
  ...
  // Initialize Meta Pixel (web only, no-op if pixel ID missing)
  initializeMetaPixel();
  
  // Capture fbc (Facebook Click ID) early for better EMQ score
  initializeFbcCapture();
...
function RootLayoutNav() {
  ...
  // WEB: Skip welcome check entirely - web goes to landing page
  React.useEffect(() => {
    if (IS_WEB) {
      setWelcomeSeen(true); // Web doesn't use welcome screens
      return;
    }
  ...
  */
  ...
  // WEB: Show landing page for unauthenticated users (no onboarding)
  // Use window.location.pathname directly to avoid flicker during navigation
  ...
  // NATIVE: Show welcome screens for first-time users (before auth)
  // OnboardingV2 handles the welcome screen (S1) and pre-auth questions
  if (!IS_WEB && !isAuthenticated && !welcomeSeen) {
  ...
  // NATIVE ONLY: Show onboarding for first-time users (only after authentication)
  // OnboardingV2 handles the post-auth questions
  const isOnboardingDisabled = process.env.EXPO_PUBLIC_DISABLE_ONBOARDING === 'true' || IS_WEB;
...
export default function RootLayout() {
  ...
  // Core providers (shared between web and native)
  const coreProviders = (children: React.ReactNode) => (
  ...
  // WEB: Skip Superwall/RevenueCat (not supported on web, use Stripe instead)
  if (IS_WEB) {
    console.log('[App] Web platform detected - skipping Superwall/RevenueCat');
    return (
      <View style={styles.container}>
        {coreProviders(<RootLayoutNav />)}
      </View>
    );
  }

  // NATIVE: Full provider stack with Superwall and RevenueCat
  const providers = (
    <QueryClientProvider client={queryClient}>
      <trpc.Provider client={trpcClient} queryClient={queryClient}>
        <ThemeProvider>
